...
locus.zoom <- function(data = NULL, snp = NA, gene = NA, region = NA, ld.file = NULL, offset_bp = 200000, genes.data = NULL, psuedogenes = FALSE, RNAs = FALSE, plot.title = NULL, plot.type = "jpg", nominal = 6, significant = 7.3, file.name = NULL, secondary.snp = NA, secondary.label = FALSE, secondary.circle = TRUE, genes.pvalue = NULL, colour.genes = FALSE, population = "EUR", sig.type = "P", nplots = FALSE, ignore.lead = FALSE, rsid.check = TRUE, nonhuman = FALSE,geneticMAP=NULL) {
  ...
  # Now re-define region to work with:
    or2<-region[2] - offset_bp*0.4
    or3<-region[3] + offset_bp*0.4
	region[2] = region[2] - offset_bp # start position
	region[3] = region[3] + offset_bp # end position
 
    if(!is.null(geneticMAP)){
        gm.sub<-geneticMAP[geneticMAP$position >=region[2] & geneticMAP$position<=region[3],]
    }
    else{
        gm.sub<-NULL
    }
  ...
  # Pull out the relevant data from the result file(s), and logBF/log p-value:
	if (is.data.frame(data)) {
	    ...
	    if (sig.type == "P") {
            ...
            stop('sig.type is not specified!')
        }
	    lead.data = data
	} else {
        if (sig.type=='P'){
            data.tmp<-lapply(data,function(x){
                x$logP<-as.numeric(unlist(lapply(x$P, elog10)))
                x
            })
        } else if(sig.type=='logBF'){
            data.tmp<-lapply(data,function(x){
		...
		data = lapply(data.tmp, function(x) subset.data(x, region))
		lead.data = data[[1]]
	}
  ...
  # Plot N locus zooms
	if (npanel > 1) {
		...
		# Set y.max:
			...
			if(all(is.null(names(data.plot)))){
                names(data.plot)<-plot.title
            }
            if(i!=npanel){
                yme<-plot.locus(data.plot = tmp.dat, plot.title = names(data.plot)[i], secondary.snp = secondary.snp, secondary.label = secondary.label, secondary.circle = secondary.circle, sig.type = sig.type, plot.var = plot.var,geneticmap=gm.sub)
                if(i==1){
                    ymd=yme
                }
            }
            if(i==npanel){
                ymq<-plot.locus(data.plot = tmp.dat, plot.title = names(data.plot)[i], secondary.snp = secondary.snp, secondary.label = secondary.label, secondary.circle = secondary.circle, sig.type = sig.type, plot.var = plot.var,yd=ymd,x.left=or2,x.right=or3,geneticmap=gm.sub)
            }
            rm(tmp.dat)
		}
	} else {
		...
		plot.var = c(y.max, x.min, x.max, lead.snp, nominal, significant)
		plot.locus(data.plot = data.plot, plot.title = plot.title, secondary.snp = secondary.snp, secondary.label = secondary.label, secondary.circle = secondary.circle, sig.type = sig.type, plot.var = plot.var,geneticmap=gm.sub)
	}
...
#### Function to create LocusZoom style plot (without gene track): ###
	plot.locus <- function(data.plot = NULL, plot.title = NULL, secondary.snp = NA, secondary.label = FALSE, secondary.circle = TRUE, sig.type = "P", plot.var = NULL,yd=NA,x.left=NA,x.right=NA,geneticmap=NULL) {
	#browser()
    # Variables:
    ...
    if(!is.null(geneticmap)){
        lines(geneticmap$position, geneticmap[['COMBINED_rate.cM.Mb.']]*y.max/100,col='tomato3',lwd=0.3)
        axis(4,col='tomato3',col.axis='tomato3',at=c(0,y.max/4,y.max/2,3*y.max/4,y.max),labels=c(0,25,50,75,100))
        mtext('Recombination rate (CM/Mb)',4,col='tomato3',line=2)
    }

    if(is.na(yd)){
        yd<-grconvertY(y.max*1,'user','device')
    }
    else{
        if(is.na(x.left) | is.na(x.right)){
            stop('x coordinates for rect region was not given')
        }
        par(xpd=NA)
        print(yd)
        rect(x.left,0,x.right,grconvertY(yd,'device','user'),col='#FF000040',border=NA)
    }
